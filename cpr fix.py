# cpr_fix_with_embedded_alpha_ooscal.py
# Production + Embedded Research Harness with strict OOS calibration (Train → Calibrate → Test)
# ---------------------------------------------------------------------------------------------
# Production:
#   - 5D volatility-adjusted cross-sectional quantile classification (Top-20% vs Bottom-20%)
#   - Optional 1D follow-through gate (supportive)
#   - Strict OOS-only probability calibration & expected-return mapping (built from TEST window only)
#   - Schema-lock from training matrix; leakage-safe early stopping; equities filter
#   - Nightly watchlist CSV/optional XLSX
#
# Research (embedded alpha, corrected target):
#   - Tradable 5D target: entry = t+1 OPEN, exit = t+5 CLOSE; ret_adj = ret / ATR% (ATR(14)/close[t])
#   - Walk-forward splits with embargo
#   - Permutation test (cross-section safe)
#   - Turnover-aware backtest (costs, turnover cap)
#   - --research-report true saves concise CSV after watchlist
...
def setup_paths(out_dir: str):
    ...
    global LOG_DIR, LOAD_ERRORS_LOG, QUARANTINE_LIST, FEATURES_SCHEMA_PATH, OOS_REPORT_PATH, CALIB_TABLE_PATH, RESEARCH_CSV_PATH
...
def train_5d_quantile_cls(panel: pd.DataFrame, feats: List[str],
    ...
    for fold_no, (tr_idx, te_idx) in enumerate(folds, start=1):
        ...
        # NO per-fold calibration here: OOS-only mapping will be built later
    ...
    # Return raw CV diagnostics (optional), but final model + calibration is fitted later
...
def fit_final_model_and_oos_calibration(panel: pd.DataFrame, feats: List[str],
    ...
    # Choose best calibrator ONLY on CAL window
    final_calib, diag = _calibrate_best_brier(base, X.iloc[i_cal], y.iloc[i_cal])

    # Apply fixed calibrator to TEST window (true OOS)
...
# ===================== Map prob -> expected returns (built from OOS TEST only) =====================
...
# ===================== Embedded Research (corrected target) =====================
HORIZON = 5
ATR_WINDOW = 14
TOP_Q = 0.20
BOTTOM_Q = 0.20
...
def build_tradable_5d_target(df: pd.DataFrame) -> pd.DataFrame:
    df = df.sort_values(["symbol", "date"]).copy()
    entry_open = df.groupby("symbol")["open"].shift(-1)          # t+1 open
    exit_close = df.groupby("symbol")["close"].shift(-HORIZON)    # t+H close
    df["ret_5d_oc"] = (exit_close - entry_open) / entry_open
    vol = df["atr_pct"].replace(0.0, np.nan)
    df["ret_adj"] = df["ret_5d_oc"] / vol
    labels = []
    for d, g in df.groupby("date", sort=True):
        if len(g) < MIN_CROSS_SECTION:
            labels.append(pd.Series(index=g.index, dtype=float))
            continue
...
def walk_forward_splits_research(dates: pd.Series, train_years=3, test_years=1):
    ...
    for i in range(train_years, len(uniq_years) - test_years + 1):
        ...
        embargo_cut = test_dates.min() - pd.Timedelta(days=EMBARGO_DAYS_RESEARCH)
        train_dates = train_dates[train_dates < embargo_cut]
        yield train_dates, test_dates


def daily_spearman_ic(df: pd.DataFrame, score_col: str) -> float:
...
def permutation_test(df: pd.DataFrame, feature_cols: List[str], base_model, n_perm: int = 20):
    rng = np.random.default_rng(RANDOM_STATE)
    real_ics = []
    for tr_dates, te_dates in walk_forward_splits_research(df["date"]):
    ...
    real_ic = float(np.nanmean(real_ics)) if len(real_ics) else np.nan

    perm_ics = []
    for _ in range(n_perm):
        shuffled = df.copy()
        shuffled["target"] = shuffled.groupby("date")["target"].transform(lambda x: rng.permutation(x.values))
        ics = []
        for tr_dates, te_dates in walk_forward_splits_research(shuffled["date"]):
        ...
        perm_ics.append(float(np.nanmean(ics)) if len(ics) else np.nan)
    return real_ic, perm_ics


def turnover_backtest(df: pd.DataFrame, score_col: str, top_n: int = 100, turnover_limit: float = 0.30) -> pd.Series:
    ...
    for d, g in df.groupby("date"):
        ...
        denom = max(1, len(prev_hold) or 1)
        turnover = (len(added) + len(removed)) / denom
...
def run_research_after_watchlist(panel: pd.DataFrame, feats: List[str], research_topn: int, research_turnover_limit: float):
    df = panel.copy()
    df["date"] = pd.to_datetime(df["timestamp"]).dt.normalize()
    required = {"date","symbol","open","high","low","close"}
    missing = [c for c in required if c not in df.columns]
    if missing:
        raise RuntimeError(f"Research requires columns {missing}")
    df = compute_atr_and_atr_pct(df)
    df = build_tradable_5d_target(df)

    _, LGBMClassifier = _check_lightgbm()
    base_model = LGBMClassifier(**_lgbm_cls_params(use_monotone=False, X_cols=feats, rnd=GLOBAL_SEED + 2026))

    df_l = df.dropna(subset=["target"]).copy()
    X_cols = [c for c in feats if c in df_l.columns]
    if not X_cols:
        raise RuntimeError("No overlapping feature columns for research.")

    real_ic, perm_ics = permutation_test(df_l, X_cols, base_model, n_perm=20)
    perm_mean = float(np.nanmean(perm_ics)) if len(perm_ics) else float('nan')
    perm_std  = float(np.nanstd(perm_ics)) if len(perm_ics) else float('nan')

    preds_all = []
    for tr_dates, te_dates in walk_forward_splits_research(df_l["date"]):
        tr = df_l[df_l["date"].isin(tr_dates)].dropna(subset=["target"])
        te = df_l[df_l["date"].isin(te_dates)].dropna(subset=["target"])
        if tr.empty or te.empty:
            continue
        m = clone(base_model)
        m.fit(tr[X_cols], tr["target"])
        te = te.copy()
        te["score"] = m.predict_proba(te[X_cols])[:, 1]
        preds_all.append(te)
    if not preds_all:
        raise RuntimeError("No predictions produced in research splits.")
    preds = pd.concat(preds_all).sort_values(["date","symbol"])  # OOS scores

    pnl = turnover_backtest(preds, "score", top_n=research_topn, turnover_limit=research_turnover_limit)
    pnl_mean = float(pd.to_numeric(pnl, errors="coerce").mean())
    pnl_std  = float(pd.to_numeric(pnl, errors="coerce").std())
    pnl_len  = int(len(pnl))

    out = pd.DataFrame([{
        "spearman_ic_real": real_ic,
        "spearman_ic_perm_mean": perm_mean,
        "spearman_ic_perm_std": perm_std,
        "turnover_pnl_mean": pnl_mean,
        "turnover_pnl_std": pnl_std,
        "turnover_pnl_n": pnl_len,
        "top_n": int(research_topn),
        "turnover_limit": float(research_turnover_limit)
    }])
    out.to_csv(RESEARCH_CSV_PATH, index=False)
    print(f"[Research] Saved concise CSV: {RESEARCH_CSV_PATH}")

# ===================== Minimal CLI =====================
def parse_cli(argv=None):
    import argparse
    p = argparse.ArgumentParser("cpr_fix_with_embedded_alpha_ooscal")
    ...
    p.add_argument("--research-report", default="false", help='Run permutation test + turnover P&L after watchlist ("true"/"false").')
    p.add_argument("--research-topn", type=int, default=100)
    p.add_argument("--research-turnover-limit", type=float, default=0.30)
...
def main(argv=None):
    ...
    # 3) Schema lock from ACTUAL training matrix (use labels mask in final fit below)
    #    We'll recompute X on that mask inside fit_final_model_and_oos_calibration; here save full feature list medians.
    #    This is acceptable since production inference reindexes with saved feature list + medians.
    ...
    final_calib, oos_df = fit_final_model_and_oos_calibration(panel, feats,
                                                              early_stopping_rounds=args.early_stopping_rounds)
    ...
    # 6) Nightly watchlist (uses final_calib and OOS-only mapping)
    export_xlsx = str(args.export_xlsx).lower() in ("true","1","yes","y","t")
    wl = nightly_watchlist(panel, feats, final_calib, calib_table, m1_gate,
                           exclude_pattern=args.exclude_pattern,
                           gate_threshold=float(args.gate_threshold),
                           export_xlsx=export_xlsx)

    # 7) Research report (optional)
    if str(args.research_report).lower() in ("true","1","yes","y","t"):
        try:
            run_research_after_watchlist(panel, feats,
                                         research_topn=int(args.research_topn),
                                         research_turnover_limit=float(args.research_turnover_limit))
        except Exception as e:
