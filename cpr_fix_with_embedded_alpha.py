...
# ============================================================================
# ORIGINAL CPR FIX CODE (UNMODIFIED)
# ============================================================================

# compare_rank_quant_v1.py
# Signal-driven pipeline for equities:
# - Primary signal: 5D Top-20% vs Bottom-20% (vol-adjusted) quantile classification
# - Supportive gate: 1D follow-through classification (optional)
# - No raw return regression
# - Schema lock from training matrix; safe validation fallback (no test leakage)
# - Watchlist includes expected 3D/5D returns and expected Sharpe via OOS decile calibration
...
def setup_paths(out_dir: str):
    ...
    global LOG_DIR, LOAD_ERRORS_LOG, QUARANTINE_LIST, FEATURES_SCHEMA_PATH, OOS_REPORT_PATH, CALIB_TABLE_PATH
...
def _prepare_panel_rows(path_obj: Path, min_ts_map: dict):
    ...
    try:
        ...
        df = score_bias(df)
        df["symbol"] = sym
        df_train = df.dropna(subset=["ret_5d_close_pct"])
        if df_train.empty:
            nrows = len(df)
...
# ===================== Label engineering (vol-adjusted 5D ranks/quantiles) =====================
def build_5d_rank_quant_labels(panel: pd.DataFrame) -> pd.DataFrame:
    panel = panel.copy()
    panel["date"] = pd.to_datetime(panel["timestamp"]).dt.normalize()

    # Volatility proxy: rolling std of daily returns (%); fallback to ATR% if needed
    ...
    # Cross-sectional percentile rank (per date) of vol-adjusted 5D forward return
    grp = panel.groupby("date")
    panel["rank_5d_pct"] = grp["ret_5d_adj"].rank(method="average", pct=True)

    # Binary quantile labels: Top-20% vs Bottom-20%; middle is NaN (neutral)
...
def _lgbm_cls_params(use_monotone: bool, X_cols: List[str], rnd: int):
    ...
    params = dict(
        ...
        reg_alpha=0.4, reg_lambda=8.0, class_weight='balanced',
        n_jobs=-1, random_state=int(rnd), verbosity=-1,
    )
    # Monotone constraints not applied by default here (feature semantics mixed)
    return params
...
# ===================== Train 5D quantile classifier (primary) =====================
def train_5d_quantile_cls(panel: pd.DataFrame, feats: List[str],
    ...
    # Build labels
    panel_l = build_5d_rank_quant_labels(panel)
    y = panel_l["top20_vs_bot20_5d"]
    mask = y.notna()
    if not mask.any():
        raise ValueError("No labeled rows for 5D quantile classification (top/bottom 20%).")
    valid_idx = panel.index[mask].to_numpy()
    folds = list(time_cv_by_timestamp(panel_l, n_splits=n_splits, embargo_days=embargo_days, target_mask=mask))

    X_full = sanitize_feature_matrix(panel_l.loc[mask, feats].copy())
    y_full = y.loc[mask].astype(int).values

    oos_prob = np.full(len(X_full), np.nan)
    oos_rank = np.full(len(X_full), np.nan)  # store realized rank for IC
    ...
    for fold_no, (tr_idx, te_idx) in enumerate(folds, start=1):
        ...
        # Train/Validation split within training fold (by time)
        tr_core_idx, val_idx = split_train_val_by_time(panel_l, valid_idx[tr_pos], val_frac=0.2, min_val=200)
        ...
        params = _lgbm_cls_params(use_monotone=False, X_cols=list(X_full.columns), rnd=GLOBAL_SEED + 300 + fold_no)
        ...
        # Save realized rank (for IC)
        oos_rank[te_pos] = panel_l.loc[mask].iloc[te_pos]["rank_5d_pct"].values

        eta.tick(f"fold {fold_no} chosen={diag['chosen']} br_iso={diag['brier_isotonic']:.5f} br_sig={diag['brier_sigmoid']:.5f}")

    # Final model on last 20% by time (with calibration)
    df_valid = panel_l.loc[mask, ["timestamp"]].copy()
    ...
    import lightgbm as _lgb_mod
    final_base = LGBMClassifier(**_lgbm_cls_params(use_monotone=False, X_cols=list(X_full.columns), rnd=GLOBAL_SEED + 999))
    callbacks = [_lgb_mod.callback.log_evaluation(period=0)]
    ...
    return final_calib, oos_prob, oos_rank, valid_idx, panel_l

# ===================== 1D follow-through classifier (supportive gate) =====================
def train_1d_followthrough(panel: pd.DataFrame, feats: List[str], margin_pct: float = CLS_MARGIN_1D,
    ...
    mask = y.notna()
    if not mask.any():
        print("[1D gate] No labeled rows; skipping.")
    ...
    # Final gate model
    final_gate = LGBMClassifier(**params)
    callbacks = [_lgb_mod.callback.log_evaluation(period=0)]
    if EARLY_STOPPING_ROUNDS and EARLY_STOPPING_ROUNDS > 0:
        callbacks.insert(0, _lgb_mod.callback.early_stopping(stopping_rounds=int(EARLY_STOPPING_ROUNDS)))
    final_gate.fit(X_tr, y_tr, eval_set=[(X_val, y_val)], eval_metric="binary_logloss", callbacks=callbacks)
    gate_calib = CalibratedClassifierCV(estimator=final_gate, method="sigmoid", cv="prefit")
    gate_calib.fit(X_val, y_val)
    return gate_calib, oos_prob, valid_idx

# ===================== OOS report + calibration table =====================
def oos_and_calibration(panel_labeled: pd.DataFrame,
                        oos5_prob: np.ndarray, oos5_rank: np.ndarray, valid_idx: np.ndarray) -> Dict[str, Dict]:
    from sklearn.metrics import brier_score_loss
    from scipy.stats import spearmanr

    # Gather realized outcomes for 3D/5D (vol-adjusted already in panel_labeled)
    idx = valid_idx
    df_oos = pd.DataFrame({
        "panel_idx": idx,
        "prob_top20_5d": oos5_prob,
        "rank_5d_pct": oos5_rank,
        "ret_3d_adj": panel_labeled.loc[idx, "ret_3d_adj"].values,
        "ret_5d_adj": panel_labeled.loc[idx, "ret_5d_adj"].values,
        "ret_3d_close_pct": panel_labeled.loc[idx, "ret_3d_close_pct"].values,
        "ret_5d_close_pct": panel_labeled.loc[idx, "ret_5d_close_pct"].values,
    })
    df_oos = df_oos.dropna(subset=["prob_top20_5d"])

    # Brier vs top/bottom label (we don’t store y here; approximate using rank tails)
    # For reporting, compute Brier against a pseudo-label (rank>=0.8 -> 1, rank<=0.2 -> 0)
    pseudo_y = np.where(df_oos["rank_5d_pct"] >= 0.8, 1, np.where(df_oos["rank_5d_pct"] <= 0.2, 0, np.nan))
    brier = float(brier_score_loss(pd.Series(pseudo_y).dropna(), df_oos.loc[~np.isnan(pseudo_y), "prob_top20_5d"]))

    # Spearman IC: predicted probability vs realized vol-adjusted 5D return
    ic5, ic5_p = spearmanr(df_oos["prob_top20_5d"], df_oos["ret_5d_adj"], nan_policy="omit")

    # Decile calibration: bucket by predicted probability
    ...
    # Convert bucket index to midpoint probability for interpolation
    probs = np.linspace(0.05, 0.95, len(calib))  # midpoints of deciles
    calib["prob_mid"] = probs
    calib["exp_sharpe_5d"] = (calib["avg_ret_5d_adj"]).astype(float)  # since ret_adj ~ ret/vol, adj mean ~ expected Sharpe proxy
    ...
    rep = {
        "5d_cls": {
            ...
            "brier_pseudo": brier,
            "spearman_ic_5d_adj": float(ic5),
            "spearman_ic_pvalue": float(ic5_p)
        }
    }
    ...
    return rep, calib_table

# ===================== Map prob -> expected returns =====================
...
def nightly_watchlist(panel: pd.DataFrame, feats: List[str],
    ...
    # Rank by expected_sharpe_5d (primary) then expected_ret_5d
    wl = wl[["symbol","timestamp","close","avg20_vol",
             "prob_top20_5d","expected_ret_3d","expected_ret_5d","expected_sharpe_5d",
             "D_atr14","D_cpr_width_pct","long_score","short_score"]].sort_values(
        ["expected_sharpe_5d","expected_ret_5d"], ascending=[False, False]
    )
...
def parse_cli(argv=None):
    import argparse
    p = argparse.ArgumentParser("compare_rank_quant_v1")
...
def main(argv=None):
    ...
    # 2) Train 5D quantile classifier (primary)
    m5_cls_calib, oos5_prob, oos5_rank, valid_idx, panel_labeled = train_5d_quantile_cls(panel, feats,
                                                                                          n_splits=args.cv_splits,
                                                                                          embargo_days=args.embargo_days,
                                                                                          early_stopping_rounds=args.early_stopping_rounds)

    # 3) Schema lock from ACTUAL training matrix (use valid mask)
    X_train = sanitize_feature_matrix(panel_labeled.loc[valid_idx, feats].copy())
    ...
    # 4) OOS report + calibration table
    rep, calib_table = oos_and_calibration(panel_labeled, oos5_prob, oos5_rank, valid_idx)
    print(json.dumps(rep, indent=2))

    # 5) Train 1D supportive gate (optional)
    m1_gate = None
    if str(args.use_1d_gate).lower() in ("true","1","yes","y","t"):
        m1_gate, oos1_gate, idx1 = train_1d_followthrough(panel, feats, margin_pct=CLS_MARGIN_1D,
                                                          n_splits=args.cv_splits, embargo_days=args.embargo_days)

    # 6) Nightly watchlist
    export_xlsx = str(args.export_xlsx).lower() in ("true","1","yes","y","t")
    wl = nightly_watchlist(panel, feats, m5_cls_calib, calib_table, m1_gate,
...
# ============================================================================
# EMBEDDED LOCKED ALPHA SPINE (AUTHORITATIVE)
# THIS OVERRIDES ALL TARGET / CV / BACKTEST LOGIC ABOVE
# DO NOT MODIFY WITHOUT REDESIGN
# ============================================================================

"""
===============================================================================
ALPHA PIPELINE – FINAL, COMPLETE, COPY-PASTE VERSION
===============================================================================
...
• Return: OPEN → CLOSE
• Volatility: ATR(14)
• Target: Cross-sectional quantile classifier
• Quantiles: Top 25% = 1, Bottom 25% = 0, Middle ignored
• Validation: Walk-forward CV with EMBARGO
• Testing: Permutation test (cross-section safe)
• Trading: Turnover-aware backtest with costs
• Philosophy: FAIL LOUDLY OR DO NOT RUN
...
# =============================================================================
# CONFIG (DO NOT CHANGE WITHOUT REDESIGN)
# =============================================================================
HORIZON = 5
ATR_WINDOW = 14
...
TURNOVER_LIMIT = 0.30          # max daily turnover
COST_BPS = 20 / 10000          # 20 bps round-trip cost
...
# FAIL LOUDLY
# =============================================================================
def fail_loudly(cond, msg):
    if not cond:
        raise RuntimeError(f"[FAIL LOUDLY] {msg}")
...
def compute_atr(df):
    ...
    df["atr"] = (
        ...
        .rolling(ATR_WINDOW, min_periods=ATR_WINDOW)
        .mean()
        .reset_index(level=0, drop=True)
    )

    fail_loudly(df["atr"].isna().sum() == 0, "ATR contains NaNs")

    return df

# =============================================================================
# TARGET CONSTRUCTION (5D OPEN→CLOSE, ATR-ADJUSTED)
# =============================================================================
def build_target(df):
    df = df.sort_values(["symbol", "date"]).copy()

    fut_open = df.groupby("symbol")["open"].shift(-HORIZON)
    fut_close = df.groupby("symbol")["close"].shift(-HORIZON)

    df["ret_5d_oc"] = (fut_close - fut_open) / fut_open
    fail_loudly(df["ret_5d_oc"].isna().sum() == 0,
                "NaNs in forward open-to-close returns")

    df["ret_adj"] = df["ret_5d_oc"] / df["atr"]
    fail_loudly(df["ret_adj"].isna().sum() == 0,
                "NaNs after ATR adjustment")
    ...
    for d, g in df.groupby("date"):
        ...
        lo = g["ret_adj"].quantile(BOTTOM_Q)
        hi = g["ret_adj"].quantile(1 - TOP_Q)

        y = pd.Series(np.nan, index=g.index)
        y[g["ret_adj"] <= lo] = 0
        y[g["ret_adj"] >= hi] = 1

        labels.append(y)

    df["target"] = pd.concat(labels).sort_index()
    fail_loudly(df["target"].notna().sum() > 0, "No targets produced")
...
# =============================================================================
def walk_forward_splits(dates, train_years=3, test_years=1):
    dates = pd.to_datetime(pd.Series(dates).unique()).sort_values()
    years = dates.year.values
    uniq_years = np.unique(years)

    for i in range(train_years, len(uniq_years) - test_years + 1):
        train_yrs = uniq_years[:i]
        test_yrs = uniq_years[i:i + test_years]

        train_dates = dates[np.isin(years, train_yrs)]
        test_dates = dates[np.isin(years, test_yrs)]

        embargo_cut = test_dates.min() - pd.Timedelta(days=EMBARGO_DAYS)
        train_dates = train_dates[train_dates < embargo_cut]
...
# DAILY SPEARMAN IC
# =============================================================================
def daily_spearman_ic(df, score_col):
    def _ic(g):
        if g[score_col].nunique() < 2:
            return np.nan
        return g["target"].corr(g[score_col], method="spearman")

    return df.groupby("date").apply(_ic).mean()

# =============================================================================
# PERMUTATION TEST (CROSS-SECTION SAFE)
# =============================================================================
def permutation_test(df, feature_cols, model, n_perm=20):
    rng = np.random.default_rng(RANDOM_STATE)

    real_ics = []

    for tr_dates, te_dates in walk_forward_splits(df["date"]):
        tr = df[df["date"].isin(tr_dates)].dropna(subset=["target"])
        te = df[df["date"].isin(te_dates)].dropna(subset=["target"])
        ...
        te = te.copy()
        te["score"] = m.predict_proba(te[feature_cols])[:, 1]

        real_ics.append(daily_spearman_ic(te, "score"))

    real_ic = np.nanmean(real_ics)

    perm_ics = []
    for _ in range(n_perm):
        shuffled = df.copy()
        shuffled["target"] = shuffled.groupby("date")["target"].transform(
            lambda x: rng.permutation(x.values)
        )

        ics = []
        for tr_dates, te_dates in walk_forward_splits(shuffled["date"]):
            tr = shuffled[shuffled["date"].isin(tr_dates)].dropna(subset=["target"])
            te = shuffled[shuffled["date"].isin(te_dates)].dropna(subset=["target"])
            ...
            te = te.copy()
            te["score"] = m.predict_proba(te[feature_cols])[:, 1]
            ics.append(daily_spearman_ic(te, "score"))

        perm_ics.append(np.nanmean(ics))

    return real_ic, perm_ics

# =============================================================================
# TURNOVER-AWARE BACKTEST
# =============================================================================
def turnover_backtest(df, score_col, top_n=100):
    df = df.sort_values(["date", score_col], ascending=[True, False]).copy()

    pnl = []
    prev_hold = set()

    for d, g in df.groupby("date"):
        longs = set(g.head(top_n)["symbol"])

        added = longs - prev_hold
        removed = prev_hold - longs
        turnover = (len(added) + len(removed)) / max(1, len(prev_hold))

        if turnover > TURNOVER_LIMIT:
            longs = prev_hold.copy()

        daily_ret = g[g["symbol"].isin(longs)]["ret_5d_oc"].mean()
        cost = turnover * COST_BPS

        pnl.append(daily_ret - cost)
        prev_hold = longs
...
# MAIN PIPELINE
# =============================================================================
def run_pipeline(df, feature_cols, model):
    df = compute_atr(df)
    df = build_target(df)
    ...
    pnl = turnover_backtest(preds, "score")
